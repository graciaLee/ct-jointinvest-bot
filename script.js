...
const CHECKLISTS = {
  school: { title: "학교 위협 - 대테러 합동조사 필요성 판단", items: [
  ...
  explosive: { title: "폭발물 위협 - 대테러 합동조사 필요성 판단", items: [
  ...
  venue: { title: "공연장·영화관·체육관 위협 - 대테러 합동조사 필요성 판단", items: [
  ...
  firearm: { title: "총기 위협 - 대테러 합동조사 필요성 판단", items: [
  ...
  intl: { title: "국제행사 위협 - 대테러 합동조사 필요성 판단", items: [
  ...
  unknown: { title: "미상물질 위협 - 대테러 합동조사 필요성 판단", items: [
...
function showChecklistOptions(){
  const btns = CHECKLIST_OPTIONS.map(o =>
    `<button type="button" class="cl-opt" data-cl="${o.id}">${o.label}</button>`
  ).join("");
